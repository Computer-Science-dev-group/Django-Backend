...
from uia_backend.notification.utils.email_tracking import (
    SendGridTrackingHandler,
    SIBTrackingHandler,
)
...
class SendGridTrackingHandlerTests(TestCase):
    def setUp(self) -> None:
        self.email_message = EmailMessageModelFactory.create(
            message_id="test-message-id",
            recipient_email="user@example.com",
    ...
    def test_email_message_status_updated_successfully(self):
        ...
        handler = SendGridTrackingHandler(event)
    ...
    def test_event_type_not_allowed(self):
        ...
        self.assertRaises(AssertionError, SendGridTrackingHandler, event=event)
    ...
    def test_successful_tracking(self):
        """
        Tests that email message status is updated
        correctly when the event type is "delivered" or "open"
        """
    ...
    def test_failed_tracking(self):
        """
        Tests that email message status is updated correctly
        when the event type is "bounce" or "rejected
        """
        ...
        handler = SendGridTrackingHandler(bounced_event)
...
class SendGrindSignalReceiverTests(TestCase):
    def setUp(self) -> None:
        self.email_message = EmailMessageModelFactory.create(
            message_id="test-message-id",
            recipient_email="user@example.com",
    ...
    def test_SendGrid_signal_processed_successfully(self):
        """Test that the signal was successfuly processed by SendGridTrackingHandler"""
        ...
        tracking.send(sender=object(), event=event, esp_name="SendGrid")
